...
def min(logger, cfgs, map, net, dataloader,  road_adj, risk_adj, poi_adj,
    ...
    for map, test_loader_ues in ziped:
        ...
        X_pgd = atc_round(X_pgd,X_pgd001)#攻击后攻击前
...
def kl_pgd(logger, cfgs, map, net, dataloader,  road_adj, risk_adj, poi_adj,
    ...
    adv_RMSE, adv_Recall, adv_MAP, adv_RCR = mask_evaluation_np(
        inverse_trans_label, inverse_trans_pre, risk_mask, 0)
    logger_info(logger, False, 'Info:  不可感知性质：{}',sum(unable/1080./7/20/20))
    logger_info(logger, False, 'Info:  [{}/{} ({:.0f}%)]\t  clean_RMSE: {:.4f} clean_Recall: {:.4f} clean_MAP: {:.4f} clean_RCR: {:.4f}   adv_RMSE: {:.4f} adv_Recall: {:.4f} adv_MAP: {:.4f} adv_RCR: {:.4f} time:{:.3f}'.format(
                batch_num_x, 1080,
                100. * batch_idx / len(dataloader),
                clean_RMSE, clean_Recall, clean_MAP, clean_RCR,
...
def JS_pgd_impr(logger, cfgs, map, net, dataloader,  road_adj, risk_adj, poi_adj,
           grid_node_map, scaler, risk_mask, device, data_type='nyc'):
    '''
    logger:日志保存
    cfgs：配置信息
    ...
    l_50 = np.load('/home/wyb/mycode/GSNet-master/array050.npy')/46.0
    ...
    for map, test_loader_ues in ziped:
        ...
        target_time = torch.zeros_like(target_time)
        road_adj = np.zeros_like(road_adj) 
        risk_adj= np.zeros_like(risk_adj) 
        poi_adj= np.zeros_like(poi_adj) 
        ...
        #####组合攻击方法
        #1.risk攻击
        feature = vers_attack001(map_01,feature,device,l_50)
        #2.时间轮转攻击
        #24Hours
...
def fgsm_impr(logger, cfgs, map, net, dataloader,  road_adj, risk_adj, poi_adj,
         grid_node_map, scaler, risk_mask, device, data_type='nyc'):
    '''
    fgsm攻击方法
    logger:日志保存
